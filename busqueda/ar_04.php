<?php
    ...
    switch ($_GET['opc_busq']) {
        case '1': {
            // solo fecha
            $str = "SELECT m.id, a.cedula, a.nombres, IF(a.nacimiento = '0000-00-00','No Presenta',DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(a.nacimiento)), '%Y')+0), a.sexo, a.procedencia, a.telefono, m.actividad, m.remitido, DATE_FORMAT(m.fecha, '%d-%m-%Y'), DATE_FORMAT(m.cita, '%d-%m-%Y'), m.observacion FROM ginecologia AS m, atendido AS a WHERE (a.cedula = m.atendido_cedula) AND (m.fecha >= '{$_GET['desd_busq']}') AND (m.fecha <= '{$_GET['hast_busq']}') ORDER BY m.id ASC;";
            break;
        }
        case '2': {
            // fecha + cedula atendido
            $str = "SELECT m.id, a.cedula, a.nombres, IF(a.nacimiento = '0000-00-00','No Presenta',DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(a.nacimiento)), '%Y')+0), a.sexo, a.procedencia, a.telefono, m.actividad, m.remitido, DATE_FORMAT(m.fecha, '%d-%m-%Y'), DATE_FORMAT(m.cita, '%d-%m-%Y'), m.observacion FROM ginecologia AS m, atendido AS a WHERE (m.atendido_cedula = '{$_GET['cedula_atn_busq']}') AND (a.cedula = m.atendido_cedula) AND (m.fecha >= '{$_GET['desd_busq']}') AND (m.fecha <= '{$_GET['hast_busq']}') ORDER BY m.fecha ASC;";
            break;
        }
        case '3': {
            // fecha + municipio
            $str = "SELECT m.id, a.cedula, a.nombres, IF(a.nacimiento = '0000-00-00','No Presenta',DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(a.nacimiento)), '%Y')+0), a.sexo, a.procedencia, a.telefono, m.actividad, m.remitido, DATE_FORMAT(m.fecha, '%d-%m-%Y'), DATE_FORMAT(m.cita, '%d-%m-%Y'), m.observacion FROM ginecologia AS m, atendido AS a  WHERE (a.procedencia LIKE '%{$_GET['municipio_atn_busq']}%') AND (a.cedula = m.atendido_cedula) AND (m.fecha >= '{$_GET['desd_busq']}') AND (m.fecha <= '{$_GET['hast_busq']}') ORDER BY m.fecha ASC;";
            break;
        }
        case '4': {
            // fecha + parroquia
            $str = "SELECT m.id, a.cedula, a.nombres, IF(a.nacimiento = '0000-00-00','No Presenta',DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(a.nacimiento)), '%Y')+0), a.sexo, a.procedencia, a.telefono, m.actividad, m.remitido, DATE_FORMAT(m.fecha, '%d-%m-%Y'), DATE_FORMAT(m.cita, '%d-%m-%Y'), m.observacion FROM ginecologia AS m, atendido AS a WHERE (a.procedencia = '{$_GET['parroquia_atn_busq']}') AND (a.cedula = m.atendido_cedula) AND (m.fecha >= '{$_GET['desd_busq']}') AND (m.fecha <= '{$_GET['hast_busq']}') ORDER BY m.fecha ASC;";
            break;
        }
        case '7': {
            // fecha + edad
           $str = "SELECT m.id, a.cedula, a.nombres, IF(a.nacimiento = '0000-00-00','No Presenta',DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(a.nacimiento)), '%Y')+0), a.sexo, a.procedencia, a.telefono, m.actividad, m.remitido, DATE_FORMAT(m.fecha, '%d-%m-%Y'), DATE_FORMAT(m.cita, '%d-%m-%Y'), m.observacion FROM ginecologia AS m, atendido AS a WHERE (a.cedula = m.atendido_cedula) AND (m.fecha >= '{$_GET['desd_busq']}') AND (m.fecha <= '{$_GET['hast_busq']}') AND IF(-1 = {$_GET['edad_busq']},a.nacimiento = '0000-00-00',DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(a.nacimiento)), '%Y')+0 = {$_GET['edad_busq']}) ORDER BY m.fecha ASC;";
            break;
        }
    }
        ...
        <table class="easyui-datagrid" <?php echo $title; ?> style="width:1196px;height:530px;">
	        <thead>
	        	<tr>
                    ...
                    <th data-options="field:'item12',width:100">CITA</th>
