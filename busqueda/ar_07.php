<?php
    ...
    switch ($_GET['opc_busq']) {
        case '1': {
            // solo fecha
            $str = "SELECT m.id, a.cedula, UPPER(a.nombres), IF(a.nacimiento = '0000-00-00','No Presenta',DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(a.nacimiento)), '%Y')+0), a.sexo, a.procedencia, a.telefono, UPPER(m.diagnostico), UPPER(m.remitido), DATE_FORMAT(m.fecha, '%d-%m-%Y') FROM pediatria AS m, atendido AS a WHERE (a.cedula = m.atendido_cedula) AND (m.fecha >= '{$_GET['desd_busq']}') AND (m.fecha <= '{$_GET['hast_busq']}') ORDER BY m.fecha ASC;";
            break;
        }
        case '3': {
            // fecha + municipio
            $str = "SELECT m.id, a.cedula, UPPER(a.nombres), IF(a.nacimiento = '0000-00-00','No Presenta',DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(a.nacimiento)), '%Y')+0), a.sexo, a.procedencia, a.telefono, UPPER(m.diagnostico), UPPER(m.remitido), DATE_FORMAT(m.fecha, '%d-%m-%Y') FROM pediatria AS m, atendido AS a WHERE (a.procedencia LIKE '%{$_GET['municipio_atn_busq']}%') AND (a.cedula = m.atendido_cedula) AND (m.fecha >= '{$_GET['desd_busq']}') AND (m.fecha <= '{$_GET['hast_busq']}') ORDER BY m.fecha ASC;";
            break;
        }
        case '4': {
            // fecha + parroquia
            $str = "SELECT m.id, a.cedula, UPPER(a.nombres), IF(a.nacimiento = '0000-00-00','No Presenta',DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(a.nacimiento)), '%Y')+0), a.sexo, a.procedencia, a.telefono, UPPER(m.diagnostico), UPPER(m.remitido), DATE_FORMAT(m.fecha, '%d-%m-%Y') FROM pediatria AS m, atendido AS a WHERE (a.procedencia LIKE '%{$_GET['parroquia_atn_busq']}%') AND (a.cedula = m.atendido_cedula) AND (m.fecha >= '{$_GET['desd_busq']}') AND (m.fecha <= '{$_GET['hast_busq']}') ORDER BY m.fecha ASC;";
            break;
        }
        case '7': {
            // fecha + edad
            $str = "SELECT m.id, a.cedula, UPPER(a.nombres), IF(a.nacimiento = '0000-00-00','No Presenta',DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(a.nacimiento)), '%Y')+0), a.sexo, a.procedencia, a.telefono, UPPER(m.diagnostico), UPPER(m.remitido), DATE_FORMAT(m.fecha, '%d-%m-%Y') FROM pediatria AS m, atendido AS a WHERE (a.cedula = m.atendido_cedula) AND (m.fecha >= '{$_GET['desd_busq']}') AND (m.fecha <= '{$_GET['hast_busq']}') AND IF(-1 = {$_GET['edad_busq']},a.nacimiento = '0000-00-00',DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(a.nacimiento)), '%Y')+0 = {$_GET['edad_busq']}) ORDER BY m.fecha ASC;";
            break;
        }
    }
